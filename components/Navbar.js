...
/**
 * Logika Global untuk menutup dropdown saat klik di luar
 */
...
export const Navbar = (currentPage) => {
  ...
  const navLinks = `
    ...
    ${isLoggedIn ? `<li><a onclick="navigateTo('${dashboardTarget}')" class="${activeClass(dashboardTarget)} font-semibold py-3">Dashboard</a></li>` : ""}
  `;

  return `
    <nav class="bg-base-100/95 backdrop-blur-sm shadow-sm w-full border-b border-base-content/10 sticky top-0 z-[100]">
      <!-- MENGGUNAKAN STANDAR TAILWIND CONTAINER LANGSUNG -->
      <div class="navbar container mx-auto px-4 md:px-10 lg:px-16">
        ...
        <div class="navbar-end gap-1 md:gap-2">
          ...
          <div class="flex items-center">
            ${
              isLoggedIn
                ? `<button onclick="handleLogout()" class="btn btn-ghost btn-sm md:btn-md">Keluar</button>`
                : `<button onclick="navigateTo('login')" class="btn btn-primary btn-sm md:btn-md px-6 rounded-xl shadow-md">Masuk</button>`
            }
          </div>
          ...
          <details class="dropdown dropdown-end lg:hidden">
            ...
            <ul class="dropdown-content menu p-4 shadow-2xl bg-base-100 rounded-l-2xl border-l border-b border-base-content/10 w-[65vw] h-[92vh] mt-4 font-bold flex flex-col gap-2 origin-right transition-all">
              ${navLinks}
            </ul>
          </details>
        </div>
